...
void Actor::setMoveSpeed(GLfloat p_moveSpeed) {
	m_moveSpeed = p_moveSpeed;
}
void Actor::setJumpHeight(GLfloat p_jumpHeight) {
...
void Actor::update(WorldData p_world, GLfloat p_deltaTime) {
	...
	if(m_onGround) {
		m_velocity.x = m_velocity.x + m_acceleration.x * (p_deltaTime * m_moveSpeed);
		m_velocity.z = m_velocity.z + m_acceleration.z * (p_deltaTime * m_moveSpeed);
		m_velocity.x /= 2.f;
		m_velocity.z /= 2.f;
	}
	else {
		m_velocity.x = m_velocity.x + m_acceleration.x * (p_deltaTime * m_moveSpeed / 5);
		m_velocity.z = m_velocity.z + m_acceleration.z * (p_deltaTime * m_moveSpeed / 5);
		m_velocity.x /= 1.1f;
		m_velocity.z /= 1.1f;
	}
