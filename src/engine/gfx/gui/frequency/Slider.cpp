...
void CSlider::input(Sint8& p_interactFlags)
{
	...
	if((p_interactFlags & EVENT_MOUSEOVER) || m_held)
	{
		...
		{
			if(_mousePos.x >= -m_width && _mousePos.x < m_length + m_width &&
				_mousePos.y >= -m_height / 2 && _mousePos.y < m_height / 2)
			{
				m_held = true;
			}
		}
		...
		if((p_interactFlags & EVENT_MOUSEOVER) && (m_held || (_mousePos.x >= -m_width && _mousePos.x < m_length + m_width &&
													   _mousePos.y >= -m_height / 2 && _mousePos.y < m_height / 2)))
		{
			addTooltip();
