...
void ColorPanel::input(Sint8& p_interactFlags, Vector2<Sint32> p_mousePos)
{
...
void SColorPanel::input(Sint8& p_interactFlags, Vector2<Sint32> p_mousePos)
{
	p_mousePos = p_mousePos - m_pos;
	if((p_interactFlags & EVENT_MOUSEOVER) && p_mousePos.x >= 0 && p_mousePos.y >= 0 && p_mousePos.x < m_size.x && p_mousePos.y < m_size.y)
	{
		p_interactFlags -= EVENT_MOUSEOVER;
		if(GKey::keyDown(GLFW_MOUSE_BUTTON_LEFT))
			callPressFunction();
	}
}
