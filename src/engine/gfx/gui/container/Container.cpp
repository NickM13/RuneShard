...
void Container::input()
{
	...
	Vector2<Sint32> mousePos = GMouse::getMousePosGui();
	input(interactFlags, mousePos);
}
void Container::input(Sint8& p_interactFlags, Vector2<Sint32> p_mousePos)
{
	if(m_visible) {
		...
		for(Sint32 i = m_componentOrder.size() - 1; i >= 0; i--) {
			...
			if(_comp->isVisible())
				_comp->input(p_interactFlags, _mousePos);
		}
	}
}
